...
import PopoverGif from "../components/popover-giphy";
...
function drawBubble(messageDict, mail, previousUser, previousTime) {
  ...
  if (
    ...
    formattedTime === formatTime(previousTime)
  ) {
...
class Chat extends React.Component {
  ...
  constructor(props) {
    ...
    const websocketUrl = urlJoin(
      process.env.REACT_APP_WEBSOCKET_BASE_URL,
      `apps/chat/v1/ws/${token}`
    );
    ...
    this.ws.onmessage = (event) => {
      ...
      me.setPreviousUserAndTime(
        messageDict["sender_mail"],
        messageDict["time"]
      );

      // Scroll to bottom only when a screen is on the bottom.
      if (
        me.chatAreaScrollPosition === "bottom" ||
        messageDict["sender_mail"] === me.state.mail
      ) {
        me.scrollToBottom();
      }
    };
  ...
  componentDidMount() {
    ...
    axios
      .get(
        urlJoin(process.env.REACT_APP_BACKEND_BASE_URL, "apps/user/v1/auth/"),
        {
      ...
      .then(function (response) {
        ...
        window.sessionStorage.setItem(
          "gaussAccessToken",
          response.data.gauss_access_token
        );
        self.setState({
        ...
        .then((res) => {
          if (res.data.length > 0) {
            res.data.forEach((message) => {
              const [previousUser, previousTime] =
                this.getPreviousUserAndTime();
              drawBubble(message, mail, previousUser, previousTime);
              this.setPreviousUserAndTime(
                message["sender_mail"],
                message["time"]
              );
            });
      ...
      <>
        ...
        <div className="chat-frame">
          <div
            ...
            onScroll={this.updateChatAreaScrollPosition}
          />
          <form
            action="src/index"
            onSubmit={this.sendMessage}
            onKeyPress={this.onCheckEnter}
          >
            <div id={"chat-gif"} />
            <InputGroup className="mb-3">
              <FormControl
                ...
                id={"chat-input"}
              />
              <PopoverGif sendGif={this.SendGIFMessage} />
              <Button
                type={"submit"}
                variant="contained"
                endIcon={<SendIcon />}
